...
MODEL_WEIGHTS = models.segmentation.DeepLabV3_ResNet101_Weights.COCO_WITH_VOC_LABELS_V1
MODEL = models.segmentation.deeplabv3_resnet101(weights = MODEL_WEIGHTS)
...
def get_matches_lk(old_frame, frame):
    ...
    mask = FC_Resnet_seg(old_frame)
    mask = Deeplab_seg(old_frame)
    masked_frame = cv.bitwise_and(old_frame,old_frame,mask=mask)
...
def Deeplab_seg(old_frame):
    # Preprocess function
    index_tensor = torch.tensor([CLASS_TO_IDX[cl] for cl in IGNORE_CLASSES], device=device)
    ...
    input_tensor = preprocess(old_frame).unsqueeze(0).to(device)
    ...
    ignore_class_probs = torch.index_select(prediction, dim=1, index=index_tensor).sum(1)[0]
    mask = (~(ignore_class_probs > 0.1)).cpu().numpy().astype(np.uint8)  # Move mask to CPU before conversion to numpy array
    return mask

def FC_Resnet_seg(old_frame):
        # Create index tensor outside the loop
    index_tensor = torch.tensor([CLASS_TO_IDX[cl] for cl in IGNORE_CLASSES], device=device)
